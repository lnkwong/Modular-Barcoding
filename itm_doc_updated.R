...
msc <- function(ms,co1=20,co2=0.6) {
   ...
   msc = ms[s1[] >= cutoff,]
   nmc = nrow(msc)
   msc2 = ms[s1[] >= co2,]            #picks only the rows that have a score over 0.60, problem when there is non within a certain metagene --> empty list
   nmc2 = nrow(msc2)

   m2 = me(msc)
   
   while (ds(m2,m1) > 1e-7) {       # m2 is median values of top correlated values, m1 is median values of all values in a metagene
...
mmsc <- function(file,c1=0,c2=0) {
   ...
   ms0 = ms0[!is.na(ms0$metagene),]       # delte the extra blank rows
   ms00 = ms0                             # make a copy of the data
   ms01 = ms0[,-1:-2]                     # get rid of the first 2 columns
   cnames = colnames(ms0)                 # column names
   genesets = unique(ms0$metagene)        # find the gene set numbers
   ...
   for (g in genesets) { 
      ms1 = ms00[ms0$metagene == g, ];    # carve out each gene set
